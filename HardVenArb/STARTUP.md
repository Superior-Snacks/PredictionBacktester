...
| `HARDVEN_BETSLIP_TRIM` | `1` | **Sidecar (hygiene):** keep the SIDE betslip empty by sweeping stray selections via its own **Remove all → Confirm** flow. Fires (a) after **every placement attempt** (real or verify-only), (b) on each **idle organic visit** to a tab, and (c) on a **periodic sweep across ALL tabs** (see `HARDVEN_BETSLIP_SWEEP_SEC`) — "if the Remove-all button is visible, press it". Interlocked with betting: the sweep skips entirely while a bet is in flight, and it only touches PENDING (unsubmitted) cards, so it can never cancel a placed bet. `0` disables all of it. |
| `HARDVEN_BETSLIP_SWEEP_SEC` | `25` | **Sidecar (hygiene):** how often the betslip sweep visits **every** open tab. Independent of organic cadence — the organic-tick trim only fires when a gesture happens to land on the tab holding the strays, so a manual slip could linger for minutes. Read-only when a slip is empty (no button ⇒ no clicks). |
| `HARDVEN_STAKE_MIN_RUNG` | `10` | **Bot (sizing):** smallest ladder rung in account currency (EUR). Below this, the trade is SKIPPED rather than placed off-ladder. Set to `5` for small supervised test bets — 5 is still a round, human stake; values below 10 add exactly one extra rung at that value (5, then 10, 20, …). |
| `HARDVEN_STAKE_MAX` | unset (no cap) | **Bot (sizing):** hard cap on the account-currency stake, applied **before** snapping. Pair with `HARDVEN_STAKE_MIN_RUNG` to pin every bet to one rung — **`HARDVEN_STAKE_MIN_RUNG=5` + `HARDVEN_STAKE_MAX=5` makes every bet exactly €5**, the supervised-test configuration. The contract count still adapts to the leg price, so the €5 buys the correct payout. |
...
| `HARDVEN_DRYRUN_UI` | unset | **Bot (proper dry-run simulation), `--dry-run` only:** `1` = each HardVen BUY first drives the **REAL Pinnacle UI verify-only** (locate the game → click the moneyline → verify the Quick Bet popover → enter the stake, **nothing placed**) via the sidecar `POST /bet/test` `submit=false`, **then** the fill is simulated. So a dry run performs the true browser placement steps end-to-end — you watch it in the managed window — with **no money** and fills/scenarios still simulated. A verify **failure** (locate miss / American odds / wrong market) fails that leg → recovery runs (faithful). Needs the **logged-in sidecar**. Ignored under `HARDVEN_LIVE_BET_PATH` or outside `--dry-run`. |
...
| `HARDVEN_WS_VERIFY_TIMEOUT_SEC` | `10` | **Bot (executor):** how long to wait for the roving tab to bring a screening-only league under live WS coverage before abandoning THAT arb window. Pre-live lines hold for tens of seconds to minutes, so ~10s is affordable; raise for slower page loads. |
| `HARDVEN_REQUIRE_WS_VERIFIED` | `1` | **Bot (executor):** never place on a HardVen leg whose price is SCREENING-ONLY (an httpx re-seed of an untabbed tail league, `wv=false`) — only on a leg confirmed on the live browser WS. On an unverified leg it logs `[EXEC SKIP] … NOT WS-verified` and skips; the telemetry has already fired `/verify` to promote that league to a live tab, so the re-opened arb executes once WS-covered. No-op in non-reader mode (wv always true). **`0` disables** — this is a PRE-FILTER against firing on a stale screening price, NOT the price safety: `_place_via_ui` reads the live Quick Bet popover at placement and refuses if the odds moved, so no bad HardVen bet fires either way. On stable PRE-LIVE lines (90s screening ≈ live) turning it off unlocks the screening-only tail arbs at the cost of a rare phantom Kalshi reversal (Kalshi fires in parallel before the HardVen navigation confirms). |
| `HARDVEN_BETS_VIA_PAGE` | `1` | **Sidecar (My Bets read):** get open bets by **navigating a tab to the account/bets page** and capturing the site's *own* `GET /0.1/bets` response — one request, fired by the site, correlated with a real page load. `/0.1/bets` is **page-specific** (unlike `/wallet/balance`, which the page polls constantly, or `/markets/straight`, which any league view fires), so calling it off-page is a correlation a server could notice. `0` = call the endpoint directly over authed REST instead (still correct, just less well-correlated). REST is the automatic fallback if the browser route fails; skipped while a bet is in flight. |
| `PINNACLE_BETS_URL` | `https://www.pinnacle.bet/en/account/bets` | **Sidecar:** the My Bets page used by the above. |
| `HARDVEN_ORGANIC_FOCUS` | `1` | **Sidecar (quality of life):** raise the OS window when the organic layer glances at a tab / the rove tab navigates. **Cosmetic only** — Playwright enables CDP focus emulation, so every page already reports `visibilityState="visible"` and `hasFocus()=true` to the site whatever the window is doing, and Playwright drives background tabs fine. On a Windows **virtual desktop** the raise is refused by the foreground-lock and merely **flashes the taskbar**. Set `0` to keep every gesture but stop the flashing. The **bet path still raises** (the tab really should be frontmost while placing). |
| `PINNACLE_WINDOW_POS` | unset | **Sidecar (quality of life):** OS position for the managed Chrome window, `"X,Y"`. Park the bot on a **second display** so it stops covering your desktop — e.g. `1920,0` for a 1080p monitor to the right of a 1920-wide primary. Works with a real monitor *or* a dummy-HDMI plug (Windows then sees a genuine second display). Pure window placement: **no fingerprint change** — same Windows Chrome, same profile, still fully headed, no re-login. |
| `PINNACLE_WINDOW_SIZE` | unset | **Sidecar (quality of life):** OS size for the managed Chrome window, `"W,H"` (e.g. `1920,1080`). Pair with `PINNACLE_WINDOW_POS` to fill the second display. |
| `HARDVEN_MONEYLINE_ONLY` | `1` | **Bot (executor):** only EXECUTE selections the book can actually place. The Pinnacle **UI** path places straight moneylines only (`lid:mid:home\|away`); a DERIVATIVE token (`…:spread:-2.5:home`, totals) is rejected at placement — so executing one fills the Kalshi leg and immediately reverses it (~2× Kalshi fees + a burnt window). Derivatives are still **detected and logged to telemetry**; only execution is gated (`[EXEC SKIP] … DERIVATIVE leg`, journal `DERIVATIVE_NOT_PLACEABLE`). Set `0` once the book can place derivatives — e.g. an **API adapter**, where a spread is just another selection id. |
| `HARDVEN_EXEC_NET_FLOOR` | `0.985` | **Bot (executor):** minimum net cost to attempt execution (the thin-margin slippage buffer; execute only when `MIN_PLAUSIBLE_NET 0.90 ≤ net ≤ this`). Default requires ~1.5¢/set. On stable pre-live lines that settle ≤~2 days, at price-taker size (no market impact), run it thin — set closer to `EXECUTION_THRESHOLD` (0.995) to capture down to any positive edge after fees; `≥ 0.995` disables the extra floor (execute any detected arb). Keep it **above** 0.90 or the executable band collapses and nothing fires. |
...
| `HARDVEN_FAVORITE_KALSHI_SPORTS` | `tennis` | **Bot (executor):** the tennis RETIREMENT/VOID hedge. Only fire when the Kalshi leg we buy holds the FAVOURITE (ask > `HARDVEN_FAVORITE_MIN`). On a mid-match retirement the sportsbook voids (stake back) while Kalshi settles the advancing player — holding the more-likely winner on Kalshi makes a void a favourable residual, not a naked loss. Skips the underdog-on-Kalshi direction for the listed sports (`[EXEC SKIP] … UNDERDOG on Kalshi`, journal `UNDERDOG_ON_KALSHI`), costing ~half of tennis arbs. Value = the STARTING state + scope: `tennis` (default; detected from the Kalshi ticker ATP/WTA/ITF) \| `all` (every pair) \| `off`/`""` (starts disabled, scope defaults to tennis). **Toggle live with the `H` key** (`[KEYS] … H=FavHedge`) — flips the gate on/off at runtime without a restart. No effect in `--telemetry` (all windows still logged). |
| `HARDVEN_FAVORITE_MIN` | `0.5` | **Bot (executor):** the favourite threshold for the gate above — the Kalshi ask on the bought side must exceed this. `0.5` = strict favourite. Raise (e.g. `0.55`) to demand a clearer favourite (stronger void hedge, fewer arbs); must be in (0,1). |
...
| `PINNACLE_RELOGIN_MIN` | `20` | **Sidecar:** reload the page every N min to re-mint the session (< the ~30-min idle logout). The reliable keepalive. `0` = off. Consider `12` for extra margin (the main board has been seen to log out near 26 min). |
| `PINNACLE_MASS_LOGOUT_REDIRECTS` | `4` | **Sidecar:** mass-logout detection. A SINGLE authed-REST guest-redirect while the reader WS is live is a stale-replay blip (re-synced, not a logout). But this many guest-redirects within `PINNACLE_MASS_LOGOUT_WINDOW` = the account genuinely logged out (board odds keep streaming off the WS because board data is public, so "odds flowing" ≠ "logged in") → forces a real re-login (reload + submit, bypassing the "recent capture = healthy" guard, which a logged-out page fools by still SENDING its dead x-session). Look for `[PINNACLE] *** LIKELY LOGOUT …`. Bet-safety meanwhile is covered by the balance gate (`/balance` guest-redirects → 0 → no buy funds). |
| `PINNACLE_MASS_LOGOUT_WINDOW` | `30` | **Sidecar:** the window (seconds) for the guest-redirect burst count above. One re-seed cycle of a dozen dead leagues trips it instantly. |
| `PINNACLE_MASS_LOGOUT_THROTTLE` | `120` | **Sidecar:** minimum seconds between forced re-logins, so a redirect storm fires the recovery once, not repeatedly. |
...
| `HARDVEN_TAB_MAX` | `12` | **Sidecar:** max concurrent DEDICATED tab-manager tabs — the coverage-vs-machine-load ceiling (each = a Chrome renderer + its odds WS). Gaps are ranked by **soonest game start** (`hardven_start_time` from pairing, `settlement_date` fallback), so the near-term games get the dedicated live-WS tabs; leagues beyond the cap are swept by the roving tail tab (soonest-first) or left uncovered if `HARDVEN_TAB_ROVE=0`. |
...
| `HARDVEN_TAB_KEEPALIVE_MIN` | `15` | **Sidecar:** per-tab keepalive. A dedicated tab is opened once and otherwise just sits — unlike the main board (reloaded every `PINNACLE_RELOGIN_MIN`) and the rove tab (navigates every dwell), nothing re-auths it, so it hits Pinnacle's ~30-min idle logout (a mass authed-REST guest-redirect + a re-seed returning 0 tokens). The manager tracks each dedicated tab's last refresh and reloads the stalest one past this age (well under the logout window; one per tick, staggered), which re-auths the tab AND re-subscribes its odds WS. Watch `[TAB-MGR] keepalive reload league …` and the `tab_idle_min=` field on the periodic line. Keep below `PINNACLE_RELOGIN_MIN` and well below ~30. |
| `HARDVEN_TAB_BOARD_RECLAIM_SEC` | `120` | **Sidecar:** dedicated tabs never DUPLICATE the featured board. At open time board leagues are already excluded; this also RECLAIMS a tab if its league LATER appears on the board (as its games approach and it gets featured) and stays there this long. The tab is closed (board now covers it) and the slot reopens for a still-uncovered league. Sustained (timer) so a transient board blip — the primary page glancing at another sport — doesn't churn tabs. Raise to reclaim more conservatively; a huge value effectively disables reclaim. |
| `HARDVEN_TAB_ORGANIC` | `1` | **Sidecar:** light PER-TAB human activity across the reader tabs (dedicated + rove) so the browser isn't 1 lively tab + N dead scraper tabs. Rotates attention: brings a random reader tab to front and does one small gesture (scroll / mouse / occasionally open-and-dismiss a bet slip). Deliberately lighter than the primary tab's organic (the session logout is held by the primary). Paused automatically during any real bet. `0` = off. |
| `HARDVEN_TAB_POPOVER_CHANCE` | `0.15` | **Sidecar:** how often a per-tab organic action is the open-and-dismiss-bet-slip gesture (vs scroll/mouse). The gesture only OPENS the Quick Bet popover (places nothing) then clicks its X — it never touches Place Bet. `0` = never do it (movement-only per-tab activity). |
