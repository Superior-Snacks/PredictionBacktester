...
| `PINNACLE_WINDOW_WS_READ` | unset | **Sidecar:** `1` = the browser-window WS READER — parse odds off the page's OWN WS (CDP, stream-reassembled MQTT) and feed the SAME cache path as the paho feed (`_apply`). Pair with `PINNACLE_DEDICATED_WS=0` so the browser WS is the only odds source (no second connection). **Coverage follows the open board** — keep a sport board (e.g. `pinnacle.bet/en/tennis/matchups/`) open; the feed goes stale if the window navigates away (until per-sport auto-nav lands). Logs `[WS-READ]` counts every 15s. |
| `PINNACLE_WINDOW_WS_TTL` | `30` | **Sidecar:** seconds since the last browser-WS odds PUBLISH before the reader feed is considered stale (`_feed_live` → false → C# clears the books). |
