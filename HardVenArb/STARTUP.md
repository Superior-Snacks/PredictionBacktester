...
| `HARDVEN_TAB_MANAGER` | unset | **Sidecar:** `1` = the LEAGUE TAB MANAGER — automatically opens one browser tab per PAIRED league the main board isn't already feeding, so the reader covers the whole paired slate (the board alone is ~25%). Needs `PINNACLE_WINDOW_WS_READ=1` + a browser session; IGNORED under `PINNACLE_LIFECYCLE` (the browser cycles per block). Reads `hardven_league_url` from cross_pairs.json (written by pair_pinnacle) and the reader's delivered matchups; opens/closes tabs as leagues appear/settle. Logs `[TAB-MGR]`. |
| `HARDVEN_TAB_MAX` | `12` | **Sidecar:** max concurrent tab-manager tabs — the coverage-vs-machine-load ceiling (each tab = a Chrome renderer + its odds WS). If gap leagues exceed this it logs and leaves the rest uncovered; raise it if the host can take more tabs. |
...
| `PINNACLE_READER_RESEED_SEC` | `90` | **Sidecar:** in pure-reader mode (`PINNACLE_DEDICATED_WS=0 PINNACLE_WINDOW_WS_READ=1`), re-fetch EVERY active league's straight markets from the **guest API** on this cadence. The browser WS is changes-only, so a stable pre-live line never re-pushes and an un-tabbed TAIL league would freeze at its one-time seed → this keeps them fresh. **Must stay comfortably below the C# `HARDVEN_BOOK_FRESH_SEC` gate (120)** so a re-seeded stable line never ages out between cycles (allow for the walk-time across all active leagues). Reader mode also now serves the **real per-token timestamp** (not a global "fresh-while-connected" stamp), so a token that genuinely stops updating (league gone, market pulled) ages out and can't produce a phantom arb on a frozen price. Guest odds are public (no session); live execution (deferred) still verifies on the authed session. |
