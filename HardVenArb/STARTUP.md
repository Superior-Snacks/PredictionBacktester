...
| `HARDVEN_BETSLIP_TRIM` | `1` | **Sidecar (hygiene):** keep the SIDE betslip empty by sweeping stray selections via its own **Remove all → Confirm** flow. Fires (a) after **every placement attempt** (real or verify-only) and (b) **while the bot idly roams tabs** — on each idle visit to a tab (board + reader tabs), if the Remove-all button is present it clears the slip (a no-op read when empty). Interlocked with betting: it can't run during a placement (organic is paused), and it only touches PENDING (unsubmitted) cards, so it can never cancel a placed bet. `0` disables all of it. |
