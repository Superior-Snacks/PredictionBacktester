...
public class SimulatedFillProfile
{
    ...
    /// <summary>Simulated round-trip ms for a HardVen FAK order.</summary>
    public int FillLatencyMsHardVen   { get; init; } = 80;
    ...
    /// <summary>
    /// Fraction of the limit price added to the simulated HardVen fill (0.01 = 1%).
    /// HardVen slippage is modeled as percentage-of-price rather than fixed cents
    /// because HardVen prices span a much wider range (0.02–0.98).
    /// </summary>
    public decimal HardVenSlippagePct { get; init; } = 0m;
    ...
    /// <summary>0–1 probability that a HardVen FAK leg fills 0.</summary>
    public double HardVenLegFailRate   { get; init; } = 0.0;
    /// <summary>
    ...
    /// <summary>
    /// Returns the simulated HardVen fill price:
    /// <c>limitPrice × (1 + HardVenSlippagePct)</c>.
    /// </summary>
    public decimal GetHardVenFillPrice(decimal limitPrice)
        => limitPrice * (1m + HardVenSlippagePct);
    ...
    /// <summary>
    /// Simulates a HardVen FAK fill for <paramref name="requested"/> shares.
    /// Returns 0m on a leg failure, a partial quantity on a partial fill,
    /// or <paramref name="requested"/> on a full fill.
    /// HardVen supports fractional shares, so the result may be non-integer.
    /// </summary>
    public decimal SimulateHardVenFill(decimal requested)
    {
        if (requested <= 0m) return 0m;
        lock (_rngLock)
        {
            if (_rng.NextDouble() < HardVenLegFailRate) return 0m;
            if (_rng.NextDouble() < PartialFillRate)
            {
                double pct = 0.20 + _rng.NextDouble() * 0.70;
