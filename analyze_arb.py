...
def analyze(rows):
    ...
    executable = [d for d in durations if d >= 1000]  # >= 1 second
    print(f"  Min:                         {min(durations):,.0f} ms")
    print(f"  Median (p50):                {p50:,.0f} ms")
    print(f"  p90:                         {p90:,.0f} ms")
    print(f"  Max:                         {max(durations):,.0f} ms")
    print(f"  Windows >= 1s (executable):  {len(executable)}/{len(rows)} ({100*len(executable)/len(rows):.0f}%)")
    ...
    # Assume: only capture arbs lasting >= 1s, capture 70% of peak profit,
