...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        var brokersAsBase = _strategyBrokers.ToDictionary(kv => kv.Key, kv => (GlobalSimulatedBroker)kv.Value);
        var startingCapitals = _strategyConfigs.ToDictionary(c => c.Name, c => c.StartingCapital);
        ...
        if (isArbMode)
        {
            strategySweeper = new ArbSweeper(sharedInstances, orderBooks, _arbEvents, _tokenNames, _clobHttpClient,
                brokersAsBase, startingCapitals, _droppedStrategies, _subscribedTokens, SubscribeNewTokens);
        }
        else
        {
            strategySweeper = new FlashCrashSweeper(apiClient, _subscribedTokens, _tokenNames, _tokenMinSizes, _tokenFeeRates, _clobHttpClient,
                SubscribeNewTokens, brokersAsBase, startingCapitals, _droppedStrategies, _lastBookUpdate, _forceSettled);
        }
        ...
        _ = Task.Run(async () =>
        {
            while (true)
            {
                ...
                if (!_quietMode && strategySweeper != null)
                {
